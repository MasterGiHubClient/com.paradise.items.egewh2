...
public class InvokerEventDelay_CasualApp : MonoBehaviour
{
    //[FormerlySerializedAs("seconds")]
    public float seconds_CasualApp;
    //[FormerlySerializedAs("onEventInvoked")]
    public UnityEvent onEventInvoked_CasualApp;


    public async void TriggerEvent()
    {
        await Task.Delay(TimeSpan.FromSeconds(seconds_CasualApp));
        onEventInvoked_CasualApp.Invoke();
